<template>
      <div class="mx-auto px-4 py-6 sm:px-6 lg:px-8 lg:pb-16">
        <div class="overflow-hidden rounded-lg bg-white shadow">
          <div class="flex flex-col divide-y divide-gray-200 lg:divide-y-0 lg:divide-x">
            <form @submit.prevent="updateProfile()" class="divide-y divide-gray-200 lg:col-span-9">
              <!-- Profile section -->
              <div class="py-6 px-4 sm:p-6 lg:pb-8">
                <div>
                  <h2 class="text-lg font-medium leading-6 text-gray-900">Profil</h2>
                  <p class="mt-1 text-sm text-gray-500">Tyto informace budou věřejně viditelné.</p>
                </div>
                ...
                <div class="mt-6 flex flex-col lg:flex-row">
                  <div class="flex-grow space-y-6">
                    <div>
                      ...
                      <div class="mt-1 flex rounded-md shadow-sm">
                        <input v-model="user.nickname" type="text" name="username" id="username" autocomplete="username" class="block w-full min-w-0 flex-grow rounded-none rounded-r-md border-gray-300 focus:border-sky-500 focus:ring-sky-500 sm:text-sm" />
                        <span class="inline-flex items-center rounded-l-md border border-r-0 border-gray-300 bg-gray-50 px-3 text-gray-500 sm:text-sm">#12345</span>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div class="mt-1">
                        <textarea v-model="user.about" id="about" name="about" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 sm:text-sm" />
                      </div>
                  ...
                  <div class="mt-6 flex-grow lg:mt-0 lg:ml-6 lg:flex-shrink-0 lg:flex-grow-0">
                    ...
                    <div class="relative hidden overflow-hidden rounded-full lg:block">
                      <img class="relative h-40 w-40 rounded-full" :src="user.avatar" alt="" />
                ...
                <div class="mt-6 grid grid-cols-12 gap-6">
                  <div class="col-span-12 sm:col-span-6">
                    ...
                    <input v-model="user.name" type="text" name="first-name" id="first-name" autocomplete="given-name" class="mt-1 block w-full rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-sky-500 sm:text-sm" />
                  </div>

                  <div class="col-span-12 sm:col-span-6">
                    <label for="last-name" class="block text-sm font-medium text-gray-700">Příjmení</label>
                    <input v-model="user.surname" type="text" name="last-name" id="last-name" autocomplete="family-name" class="mt-1 block w-full rounded-md border border-gray-300 py-2 px-3 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-sky-500 sm:text-sm" />
                  </div>
                </div>
              </div>
...
export default defineComponent({
  setup() {
    ...
    onMounted(async () => {
      try {
        const response = await Api.userSettings(userStore.userId);
